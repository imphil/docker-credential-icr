...
impl TokenStore {
    ...
    pub fn store_token(&self, token: &StoredToken) -> Result<()> {
        ...
        // Store access token as bytes (avoids UTF-16 encoding overhead)
        ...
        // Store refresh token as bytes if present
    ...
    pub fn get_token(&self) -> Result<Option<StoredToken>> {
        ...
        // Try to get access token as bytes
        let access_entry = self.get_access_token_entry()?;
        ...
        // Try to get refresh token as bytes (optional)
        let refresh_entry = self.get_refresh_token_entry()?;
